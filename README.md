...
3. Andá a la pestaña **Actions** del repo, entrá a la corrida que terminó
   ...
   página vas a encontrar dos artifacts para descargar:
   - **TaskTracker-Windows-Portable**: un único `.exe` suelto, sin instalador.
     Lo copiás donde quieras (pendrive, cualquier carpeta) y lo ejecutás
     directo — esta es la versión portable, equivalente a la que tenías con
     Electron.
   - **TaskTracker-Windows-Installers**: el instalador tradicional (NSIS
     `.exe` + `.msi`), para quien prefiera instalarla como cualquier
     programa de Windows, con acceso directo en el menú Inicio, etc.
